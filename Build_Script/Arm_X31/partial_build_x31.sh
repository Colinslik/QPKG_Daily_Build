...
function Build_RTRR_BIN(){
	...
	make clean 1>/dev/null 2>/dev/null	
	make RECYCLE_EX=yes QNAP_HAL_SUPPORT=yes STORAGE_V2=yes ENABLE_LIMITRATE=yes LDAP=yes ENABLE_PRINTF=yes TARGET_PREFIX=${git_src_model}build/RootFS >> $log_path$log_name 2>&1
...
function Update_RTRR_TO_NAS(){
	...
	file_list=("qsyncrequest.cgi" "extdriverequest.cgi")	
...
function Update_RTRR_TO_QPKG(){

        #update libgeneral      
        cd $git_src$rtrr_library_path
        scp libgeneral.so.0.0 admin@$Dest_Server:/share/HDA_DATA/.qpkg/QDK/QPKG_4.2.0/arm-x31/base/

        #update libqsync
        cd $git_src$rtrr_libqsync_path
        scp libqsync.so.0.0 admin@$Dest_Server:/share/HDA_DATA/.qpkg/QDK/QPKG_4.2.0/arm-x31/base/

        #update RTRR bin
        cd $git_src$qsync_path
        scp qsync admin@$Dest_Server:/share/HDA_DATA/.qpkg/QDK/QPKG_4.2.0/arm-x31/base/
        ...
        for filename in "${file_list[@]}"
        do
                scp $filename admin@$Dest_Server:/share/HDA_DATA/.qpkg/QDK/QPKG_4.2.0/arm-x31/base/
        done
}
...
export TARGET_PREFIX=/opt/cross-project/arm/mindspeed/target-arm_v7-a_glibc-2.14.1_eabi
	...
	QTS4_2_0=/mnt/vdisk/dbg_4.2.0/
	Main_Trunk=/root/WorkSpace/4.2.0/NasX86/
	
	git_src=$QTS4_2_0
	git_src_model=/mnt/vdisk/dbg_4.2.0/Model/$model_name/
	
	#3. destinaion NAS
	Server_431=10.64.102.33
	Server_470B=172.17.28.66
	Server_QPKG=172.17.31.60
	Dest_Server=$Server_QPKG
	
	#4. log path
	log_path=/mnt/vdisk/walter/log/
