...
export TARGET_PREFIX=/opt/cross-project/x86/sys-root
...
export TARGET_PREFIX=${git_src_model}build/RootFS
	cd $git_src$qsync_path
	echo "===========Start make qsync ... ========" >> $log_path$log_name 2>&1
	echo " " >> $log_path$log_name 2>&1

	make clean 1>/dev/null 2>/dev/null	
	make RECYCLE_EX=yes QNAP_HAL_SUPPORT=yes STORAGE_V2=yes ENABLE_LIMITRATE=yes LDAP=yes QTS_SNAPSHOT=yes >> $log_path$log_name 2>&1
	#make RECYCLE_EX=yes QNAP_HAL_SUPPORT=yes STORAGE_V2=yes ENABLE_LIMITRATE=yes LDAP=yes ENABLE_PRINTF=yes QTS_SNAPSHOT=yes >> $log_path$log_name 2>&1
...
function Set_Environment(){
	...
	QTS4_2_0=/mnt/vdisk/4.2.0_hal/
	Main_Trunk=/root/WorkSpace/4.2.0/NasX86/
	
	git_src=$QTS4_2_0
	git_src_model=${git_src}Model/$model_name/
	
	#3. destinaion NAS
	Server_470A=172.17.30.137
	Server_470B=172.17.28.66
	Server_670A=172.17.28.218
	Dest_Server=$Server_470A
...
export KERNEL_INCLUDE=${git_src_model}../../Kernel/linux-3.12.6/include/uapi\ -I${git_src_model}../../Kernel/linux-3.12.6/include
